...
import numpy as np
# APPROACH
approach_red_conf = np.array([0.0, 0.5, -2.0, 0.0, 0.0, 0.0])
#new approach_red_conf = np.array([0.3, -0.5, 1.1781, -0.0345, -0.6, 0.0])
# GRASP
grasp_red_conf = np.array([0.0, 0.5, -2.0, 0.0, 0.0, 0.0])
grasp_blue_conf = np.array([0.05, 0.5, -2.0, 0.0, 0.0, 0.0])
# DISPOSE
dispose_red_conf = np.array([-0.7, 0.3, -1.5, 0.0, 0.0, 0.0])
dispose_blue_conf = np.array([-0.7, 0.3, -1.5, 0.0, 0.0, 0.0])
# RESET
...
@nrp.Neuron2Robot()
def arm_control(t, command, last_command_executed, approach_red_conf,
